...
# Use `compgen -k` to build these list
CMD_KWS:set[str] = {
    "break", "case", "continue", "do", "done", "elif",
    "else", "esac", "fi", "for", "function", "if", "select",
    "then", "until", "while", "return",
}
NON_CMD_KWS:str[str] = {"coproc", "time"}
KEYWORDS:set[str] = CMD_KWS | NON_CMD_KWS # "in" is special
...
class ColourConfig(BaseColourConfig):
    ...
    def __init__(self, kwargs:dict[str:dict[str:str]]={}) -> ColourConfig:
        super().__init__({
                           ...
                           "process-substitution": dict(foreground="yellow"),
...
NUMBER_REGEX:str = r"[0-9]+"
NUMBER_REGEX:re.Pattern = re.compile(NUMBER_REGEX)

# Space before the "#" is needed to make sure we don't capture "abc --def#"
    ...
    def read(self) -> None:
        ...
        if token in KEYWORDS: # Keywords
            ...
            while True:
                ...
                if not (self.isidentifier(token) or (token == "-")): break
                full_token += token
        ...
        elif token.lstrip(" \t").startswith("#"): # Comment
            if token == "#":
                # Use "self.prev_token_line" instead?
                start:Location = self.tell()
                prev_token:Token = self.token_at(self.prev_start(start))
                if prev_token[-1:] not in " \t\n":
                    self.skip()
                    return None
