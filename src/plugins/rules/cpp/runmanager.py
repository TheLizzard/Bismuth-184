...
class RunManager(BaseRunManager):
    ...
    def compile(self, *, print_str:str="") -> bool:
        ...
        command:list[str] = ["g++", *WARNINGS, "-o", "{tmp}/executable", "-O3",
                             *files, "-lm", "-funroll-all-loops", "-std=c++20"]
        if DEBUG_MODE:
            command += ["-g", "-rdynamic"]
    ...
    def get_c_files(filepath:str, results:set) -> None:
        if filepath.endswith(".cpp"):
            header, c = filepath.removesuffix(".cpp")+".h", filepath
        else:
            header, c = filepath, filepath.removesuffix(".h")+".cpp"
        if c in results:
            return None
        results.add(c)
